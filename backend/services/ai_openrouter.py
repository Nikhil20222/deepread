...
def generate_summary_openrouter(text, mode="detailed"):
...
def generate_quiz_openrouter(text):
...
def generate_mindmap_openrouter(text):
...
def generate_section_summary_openrouter(text):
...
def generate_chat_answer_openrouter(text, question, history=None):
